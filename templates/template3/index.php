    ...
    <div class="flex items-center gap-9">
        ...
        if (!empty($categories) && is_array($categories)):
            foreach ($categories as $category): 
                if (!empty($category['menu_items']) && is_array($category['menu_items']) && $category['is_active']):
        ?>
            <a class="text-white text-sm font-medium hover:text-primary transition-colors" href="#<?php echo htmlspecialchars($category['slug']); ?>"><?php echo htmlspecialchars($category['name']); ?></a>
        <?php 
                endif;
            endforeach;
        endif;
...
if (!empty($categories) && is_array($categories)):
    foreach ($categories as $category): 
        if (!empty($category['menu_items']) && is_array($category['menu_items']) && $category['is_active']):
?>
<a href="#<?php echo htmlspecialchars($category['slug']); ?>" onclick="toggleCategoryMenu()" class="text-white py-2 px-4 rounded-lg hover:bg-slate-800 transition-colors"><?php echo htmlspecialchars($category['name']); ?></a>
<?php 
        endif;
    endforeach;
endif;
...
<?php if (!empty($restaurant['whatsapp_link'])): ?>
<a href="<?php echo htmlspecialchars($restaurant['whatsapp_link']); ?>" target="_blank" class="flex min-w-[140px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-12 px-8 bg-white/90 backdrop-blur-sm text-slate-900 text-base font-bold hover:bg-white transition-all hover:scale-105">
<span class="truncate">Order Online</span>
</a>
<?php endif; ?>
</div>
...
<a class="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center hover:bg-primary transition-colors" href="<?php echo htmlspecialchars($restaurant['instagram_url']); ?>" target="_blank" title="Instagram">
<span class="text-sm font-bold">IG</span>
</a>
<?php endif; ?>
<?php if (!empty($restaurant['facebook_url'])): ?>
<a class="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center hover:bg-primary transition-colors" href="<?php echo htmlspecialchars($restaurant['facebook_url']); ?>" target="_blank" title="Facebook">
<span class="text-sm font-bold">FB</span>
</a>
<?php endif; ?>
<?php if (!empty($restaurant['twitter_url'])): ?>
<a class="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center hover:bg-primary transition-colors" href="<?php echo htmlspecialchars($restaurant['twitter_url']); ?>" target="_blank" title="Twitter">
