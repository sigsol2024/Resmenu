...
if ($method === 'GET') {
    switch ($action) {
        ...
        case 'day_detail':
            ...
            try {
                $stmt = $pdo->prepare("
                    SELECT id, reservation_time, party_size, guest_name, status, is_walkin, deposit_amount, deposit_paid
                ...
                $stmt->execute([$restaurantId, $date]);
                $reservations = $stmt->fetchAll(PDO::FETCH_ASSOC);
                foreach ($reservations as &$r) { $r['is_walkin'] = 0; }
            }
            echo json_encode([
                'success' => true,
...
if ($method === 'POST') {
    switch ($action) {
        ...
        case 'add_walkin':
            ...
            try {
                $stmt = $pdo->prepare("
                    INSERT INTO table_reservations (restaurant_id, reservation_date, reservation_time, party_size, guest_name, guest_email, guest_phone, status, is_walkin, deposit_amount, deposit_paid)
                    VALUES (?, ?, ?, 1, ?, '', '', 'confirmed', 1, 0, 0)
                ");
                $stmt->execute([$restaurantId, $date, $time, $guestName]);
            } catch (PDOException $e) {
